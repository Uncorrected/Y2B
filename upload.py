...
def get_gist(_gid, token):
    ...
    try:
        ...
        rsp.raise_for_status()  # 主动抛出异常，如果状态码不是 200
        _data = rsp.json()
        ...
        elif rsp.status_code == 403 or rsp.status_code == 401:
            raise Exception("github TOKEN 错误")
        else:
            raise Exception(f"获取 gist 数据时出现未知错误: {e}")
    return c, t, {}
...
def update_gist(_gid, token, file, data):
    try:
        ...
        if rsp.status_code == 404:
            raise Exception("gist id 错误")
        elif rsp.status_code == 422:
            raise Exception("github TOKEN 错误")
        else:
            raise Exception(f"更新 gist 数据时出现未知错误: {e}")
